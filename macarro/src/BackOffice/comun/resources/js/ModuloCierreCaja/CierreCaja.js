...
function ajustarLongitud(ingresos){

    var posicion;

    for (var i = 0; i < ingresos.length; i++) {

        posicion = ingresos[i].indexOf('.');

        if (posicion == -1 && ingresos[i] != 'x')

            ingresos[i] = ingresos[i] + '.00'
...
function obtenerDivisor(ingresos) {
    ...
    for (var i = 0; i < ingresos.length; i++) {
        ...
        if (ingresos[i] != 'x')
            if (numeroDigitos < ingresos[i].length - 3)
                numeroDigitos = ingresos[i].length - 3;
    }
...
function generarGrafica() {
    ...
    ingresos = ajustarLongitud(ingresos);
...
function generarGraficaMes() {
    ...
    //Ajustar ingresos
    var ingresoMesActual = ingresos[ingresos.length - 1];
    ingresos.pop();
    ingresos = ingresos.reverse();
    ingresos.push(ingresoMesActual);
