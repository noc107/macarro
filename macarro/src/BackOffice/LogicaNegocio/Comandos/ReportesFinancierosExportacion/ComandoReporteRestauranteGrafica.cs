...
{
    public class ComandoReporteRestauranteGrafica : Comando<string[], string>
    {
        ...
        /// necesarios para realizar el reporte de los ingresos por el puesto de 
        /// restaurante, el metodo crea un string builder que contendrá el codigo scrip html
        ...
        /// al utilizar el metodo GetDatosIngresosRestaurante.
        /// </summary>
        ...
        public override string Ejecutar(string[] n)
        {
            ...
            try
            {
                _dataTable = _dao.GetDatosIngresosRestaurante(_fi, _ff);
                _stringBuilder.Append(@"<script type=*text/javascript*> 
                                    ...
                                    data.addColumn('number', 'Restaurante');");

                for (int i = 0; i <= _dataTable.Rows.Count - 1; i++)
                {
                    _stringBuilder.Append(@" data.addRows([
                [ new Date(" + _dataTable.Rows[i]["fecha"].ToString() +
                    ")," + _dataTable.Rows[i]["ingreso"].ToString() + " ]    ]);");
                }

                _stringBuilder.Append(@" var chart = new google.visualization.ImageLineChart(
                                    document.getElementById('restaurante'));");
