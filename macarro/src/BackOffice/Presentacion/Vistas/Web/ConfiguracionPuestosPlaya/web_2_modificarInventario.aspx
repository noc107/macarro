<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="web_2_modificarInventario.aspx.cs" UnobtrusiveValidationMode="None"
    Inherits="BackOffice.Presentacion.Vistas.Web.ConfiguracionPuestosPlaya.web_2_modificarInventario"
    MasterPageFile="~/Presentacion/Vistas/Temp/back_office_temp.Master"%>

<%@ Register Src="~/Presentacion/Vistas/Web/ConfiguracionPuestosPlaya/componentes/formularioModificarInventario.ascx" TagPrefix="uc1" TagName="formularioModificarInventario" %>
<%@ Register Src="~/Presentacion/Vistas/Web/ConfiguracionPuestosPlaya/componentes/mensajeDeEstado.ascx" TagPrefix="uc1" TagName="mensajeDeEstado" %>



<asp:Content ID="Content2" ContentPlaceHolderID="css_place_holder" runat="server">
    <link href="../../../../comun/resources/css/ConfiguracionPuestosPlaya/puestosPlaya.css" rel="stylesheet" />
</asp:Content>
<asp:Content ID="Content1" ContentPlaceHolderID="title_place_holder" runat="server">
    Modificar Inventario
</asp:Content>
<asp:Content ID="Content6" ContentPlaceHolderID="title_shown_place_holder" runat="server">
    <!--- titulo de la ventana --->
    Gestion de playa
</asp:Content>
<asp:Content ID="Content8" ContentPlaceHolderID="middle_place_holder" runat="server">   
    <!--- contenido de la ventana --->
    <h2>Modificar Inventario</h2>
    <div>
        
        <asp:ScriptManager ID="scriptManager" runat="server"></asp:ScriptManager>
        <asp:UpdatePanel ID="UpdatePanel1" runat="server">
            <ContentTemplate>
        <uc1:mensajeDeEstado runat="server" id="mensajeDeEstado" />
        <uc1:formularioModificarInventario runat="server" id="formularioModificarInventario" />
                </ContentTemplate>
             <Triggers>
                <asp:AsyncPostBackTrigger ControlID="botonAceptar" />
            </Triggers>
            </asp:UpdatePanel>
        <br />
    </div>
        <div>
        <table>
            <tr>
                <td>
                    <div class="btn_aceptar_posicion">
                <asp:Button ID="botonAceptar" OnClick="botonAceptar_Click" runat="server" Text="Aceptar" CssClass="Boton"/>
            </div>
                </td>
                <td>
                    <div class="btn_aceptar_posicion">
                <asp:Button ID="botonCancelar" OnClick="botonCancelar_Click" CausesValidation="False" runat="server" Text="Cancelar" CssClass="Boton"/>
            </div>
                </td>
            </tr>
        </table>
        
    </div>
</asp:Content>