...
AS
	BEGIN
	DECLARE @Estado int;
	--Datos tupla del puesto
	DECLARE @Columna int;
	DECLARE @Fila int;
	DECLARE @DescripcionPue nvarchar(100);
	DECLARE @PrecioPue float;
	DECLARE @FKEstadoPue int;
	DECLARE @FKPlayaPue int;
	------------
	...
	BEGIN
		--Datos Necerarios para completar tupla en puesto
		...
		SET @DescripcionPue = (Select PUE_descripcion from PUESTO WHERE FK_inventario = @_invId);
		SET @PrecioPue = (Select PUE_precio from PUESTO WHERE FK_inventario = @_invId);
		SET @FKEstadoPue = (Select FK_estado from PUESTO WHERE FK_inventario = @_invId);
		SET @FKPlayaPue = (Select FK_playa from PUESTO WHERE FK_inventario = @_invId);
		-------------------------------------------------------
		...
		--Insertar puesto con un inventario nuevo
		INSERT INTO PUESTO(PUE_columna,PUE_fila,PUE_descripcion,PUE_precio,FK_estado,FK_inventario,FK_playa)
		VALUES(@Columna, @Fila, @DescripcionPue, @PrecioPue, @FKEstadoPue, @InvIDNuevo, @FKPlayaPue);

		--Actualizo en reserva_puesto el la fk del inventario viejo en el puesto por el fk del inventario nuevo
		UPDATE RESERVA_PUESTO
		SET		FK_inventario = ISNULL(@InvIDNuevo,FK_inventario)
		WHERE	FK_inventario = ISNULL(@_invId,FK_inventario);

		--Elimino en puesto tupla con el inventario viejo
		DELETE FROM PUESTO
		WHERE	FK_inventario = ISNULL(@_invId,FK_inventario);

		--Actualizo el estado del inventario nuevo a asignado
		UPDATE	INVENTARIO
		SET		FK_estado = ISNULL(@Estado,FK_estado)
		WHERE	INV_id = ISNULL(@InvIDNuevo,INV_id);
	
	END;
		-- se actualizan los datos del inventario viejo	
		UPDATE	INVENTARIO
