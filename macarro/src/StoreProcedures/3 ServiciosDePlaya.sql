...
INSERT INTO HORARIO (HOR_id, HOR_inicio, HOR_fin, FK_servicio) VALUES (2, Convert(VARCHAR(10),'02:00 pm',108), Convert(VARCHAR(10),'04:00 pm',108), 1);
	...
	ELSE
		BEGIN
			...
			WHERE ( ( UPPER(SER_nombre) LIKE UPPER(@_busqueda) + '%') 
				...
				OR( UPPER(SER_descripcion) LIKE UPPER(@_busqueda) + '%') 
				OR ( UPPER(SER_descripcion) LIKE '%' + UPPER(@_busqueda))
				OR ( UPPER(SER_descripcion) LIKE '%' + UPPER(@_busqueda) + '%')) AND FK_estado = @estado AND (EST_id = FK_estado);
